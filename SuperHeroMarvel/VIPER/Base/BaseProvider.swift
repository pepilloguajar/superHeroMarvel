...
class BaseProvider: NSObject {
    ...
    internal func getProviderData(dto: ProviderDTO,
        ...
        if loader {
            ...
            SuperHeroMarvelLoader.showProgressHud(fullScreen: true)
        }
            ...
            request.responseJSON { response in
                ...
                if (200..<300).contains(response.response?.statusCode ?? 0) {
                    ...
                    if let error = response.result.error as NSError? {
                        ...
                        print("Fecha Respuesta: \(Date().format(format: "dd/MM/yyyy-HH:mm:ss"))")
                        print("URL: \(endpoint)")
                        print(NSError(domain: "SuperHeroMarvel", code: error.code, userInfo: nil))
                        print("*************************** ACABADA ***************************")
                    }
    ...
    private func hideLoader(loader: Bool) {
        ...
        if loader {
            SuperHeroMarvelLoader.hideProgressHud()
        }
    }
    ...
    fileprivate func apiResponseError(loader: Bool = true,
        ...
        case .success:
            ...
            if let error = response.response {
                ...
                failure(NSError(domain: "SuperHeroMarvel", code: error.statusCode, userInfo: nil))
            }
            
        case .failure(let error):
            
            failure(NSError(domain: "SuperHeroMarvel", code: error._code, userInfo: nil))
        }
    }
